...
async function run(): Promise<void> {
  ...
  console.log('>>>7) searching for the page...');
  const title = data.title;
  const spaceKey = 'CS';
  const parentId = 1612939516;
  const page = await searchPage(title!, spaceKey, parentId);
  if(page) {
    console.log("Page exists, updating...");
    updatePage(Number(page.id), title!, rendered);
  } else {
    console.log("Page does not exist, creating...");
    createPage(spaceKey, title!, rendered, parentId);
  }
