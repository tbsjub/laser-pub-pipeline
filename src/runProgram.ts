...
function validatePageData(data: Record<string, string>): void {
  const requiredFields = ['pvs', 'dateRange', 'days', 'title'];
...
function validateRunDays(daysString: string): number[] {
  const days = daysString.split(',').map((day: string) => {
  ...
  if (days.length === 0) {
    throw new Error('No valid days specified in days field');
  }
...
// Function to render EJS template and create/update the final page
async function renderAndUpdatePage(data: Record<string, string>, run_days: number[]): Promise<void> {
  ...
  try {
    ...
    console.log('>>> 5) Searching for existing page...');
    ...
    if (page) {
      console.log(`Page exists, updating id ${page.content.id}...`);
      try {
        await updatePage(Number(page.content.id), title, rendered);
      ...
      try {
        await createPage(spaceKey, title, rendered, parentId);
...
function imageToBase64(imagePath: string): string {
  try {
    ...
    console.warn(`Could not load image: ${imagePath}. Error: ${error}`);
...
run().catch((err) => {
  ...
  console.error('- Check that the Confluence page contains all required fields (pvs, dateRange, days, title)');
